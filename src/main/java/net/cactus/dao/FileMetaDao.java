...
public interface FileMetaDao {
    ...
    @Select({"select * from T_SYSTEM_FILE order by createtime desc"})
    List<FileMeta> findByPage();

    @Select({"select count(*) from T_SYSTEM_FILE"})
    int count();

    @Select({"select * from T_SYSTEM_FILE where filename like concat('%', #{keyword}, '%') order by createtime desc"})
    List<FileMeta> searchByPage(@Param("keyword") String keyword);
